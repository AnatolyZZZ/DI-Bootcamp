...
CREATE TABLE DEPARTMENT ( 
  DEPTCODE INTEGER NOT NULL DEFAULT nextval('code') PRIMARY KEY, 
  DeptName CHAR(30), 
...
SELECT empfname, emplname, job, location FROM EMPLOYEE INNER JOIN DEPARTMENT ON EMPLOYEE.department_code = DEPARTMENT.deptcode
...
SELECT empfname, emplname, department_code, deptname, location FROM EMPLOYEE INNER JOIN DEPARTMENT ON EMPLOYEE.department_code = DEPARTMENT.deptcode ORDER BY empfname ASC
-- Display EMPFNAME and "TOTAL SALARY" for each employee (commission and salary)
SELECT empfname, commission + salary AS TOTAL FROM EMPLOYEE
-- Display MAX SALARY from the EMPLOYEE table.
SELECT empfname, emplname, salary FROM EMPLOYEE WHERE salary = (SELECT MAX(salary) FROM EMPLOYEE )
...
SELECT * FROM EMPLOYEE WHERE empcode = 9777