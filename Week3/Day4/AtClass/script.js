...
const startZone = document.querySelector("#section2");
startZone.addEventListener("drop", dropOnTarget);
startZone.addEventListener("dragover", overTarget);

retrieveZonesAndAddEvents("drop", dropOnStart);
...
function dropOnStart (evt) {
    evt.preventDefault();  //necessary
    const data = evt.dataTransfer.getData("text/plain");
    const elem = document.getElementById(data);
    evt.target.appendChild(elem);
    elem.classList.remove("startDrag");
}